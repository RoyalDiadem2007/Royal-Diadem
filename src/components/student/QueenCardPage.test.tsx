...
describe('My Queen Card', () => {
  it('shows her card: mark selected, proud-of, goal with status, strengths', async () => {
    ...
    expect(screen.getByRole('radio', { name: 'Hair: Afro' })).toHaveAttribute(
      'aria-checked',
      'true',
    );
  ...
  it('builds her avatar and saves it with proud-of together', async () => {
    ...
    await user.click(screen.getByRole('radio', { name: 'Eyes: Wide' }));
    await user.click(screen.getByRole('radio', { name: 'Mouth: Grin' }));
