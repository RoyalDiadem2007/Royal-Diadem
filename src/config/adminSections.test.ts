...
describe('admin section registry', () => {
  ...
  it('keeps Crown Checks super_admin-only until mentor assignment exists (OD-6)', () => {
    const crownChecks = ADMIN_SECTIONS.find((s) => s.id === 'crown-checks');
    expect(crownChecks?.roles).toEqual(['super_admin']);
    expect(sectionsForRole('mentor').some((s) => s.id === 'crown-checks')).toBe(false);
